...
SELECT * FROM employees WHERE department_id IS NULL;    -- Kimberely -> 부서에 소속되지 않음
...
-- 2) 1)번의 결과보다 늦게 입사한 직원의 목록을 추출하는 쿼리
...
-- 조건 2. 수잔의 입사일보다 hire_date가 늦게 입사한 조건
SELECT first_name, salary, hire_date
FROM employees
WHERE salary > (SELECT MEDIAN(salary) FROM employees)
    AND hire_date > (SELECT hire_date FROM employees WHERE first_name = 'Susan')
...
-- 자신이 속한 부서에 평균 급여보다 많이 받는 직원의 목록을 구하라는 의미
...
--2. 1번 쿼리에서 나온 department_id, max(salary)값을 이용해 외부 쿼리를 작성
...
-- 2. 1번 쿼리를 활용, rownum값까지 확인, rownum <= 5이하인 레코들 -> 상위 5개의 레코드
